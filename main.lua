...
VERSION_TEXT = {{0.4, 0.4, 0.4}, "VERSION 1.0.2"}
...
BESTIARY = {
	...
	["spider"] = {speed = 16.0, spf = 0.2, points = 10, cooldown = 0.0, collision = "enemy", hits = 1, level1 = 5, eachLevel = 1, debutLevel = 1},
	["wasp"] = {speed = 8.0, spf = 0.05, points = 25, cooldown = 3.0, steps = 5, collision = "enemy", hits = 1, level1 = 3, eachLevel = 1, debutLevel = 1},
	["turret"] = {speed = 0.0, spf = 0.25, points = 0, cooldown = 10.0, collision = "invulnerable", hits = 0, level1 = 3, eachLevel = 1, debutLevel = 1},
	["skull"] = {speed = 16.0, spf = 0.25, points = 0, cooldown = 0.0, collision = "omnipotent", hits = 0},
	["tank"] = {speed = 64.0, spf = 0.15, points = 100, cooldown = 3.0, steps = 10, collision = "enemy", hits = 10, level1 = 2, eachLevel = 0.5, debutLevel = 2},
	["launcher"] = {speed = 24.0, spf = 0.25, points = 75, cooldown = 10.0, steps = 5, collision = "enemy", hits = 3, level1 = 2, eachLevel = 1, debutLevel = 2},
...
function placeRandomEnemies()
	...
	for k in pairs(BESTIARY) do
		if BESTIARY[k].debutLevel then
			if level >= BESTIARY[k].debutLevel then
				table.insert(keys, k)
			end
		end
	end
...
function resetEnemies()
	local mapX, mapY
	for i, c in ipairs(combatants) do
		if c.name == "flame" or c.name == "skull" or c.name == "rocket" then
			c.destroyed = true
		elseif c.name == "turret" or c.name == "slider"
			or c.name == "battery" or c.name == "boss" or c.name == "shield" then
			c.hits = BESTIARY[c.name].hits
		elseif c.name ~= "player" then
			mapX, mapY = findVacantSpot(2, 2, MAP_SIZE - 2, MAP_SIZE - 2, BESTIARY[c.name].speed == 0.0)
			c.x = mapX * TILE_SIZE
			c.y = mapY * TILE_SIZE
			c.dX = c.x
			c.dY = c.y
			...
			c.frame = 1
			c.waiting = true
			c.cooling = math.random() * BESTIARY[c.name].cooldown
			c.stepping = 0
			c.flashing = 0
			c.hits = BESTIARY[c.name].hits
		end
